%-Load Range-------------------------------------------------
% R = loadRange(currency, start_date, end_date, fix_data)
%
...
%------------------------------------------------------------
function R = DRLoadRange(currency, start_date, end_date, fix_data,quiet)
n_start = datenum(splitDate(start_date));
n_end = datenum(splitDate(end_date));

assert(length(start_date) == length(end_date),'different timescales between start and end date');
assert(strcmp(currency,'GBPUSD') || strcmp(currency,'EURUSD'),'invalid currency. [GBPUSD, EURUSD]');
assert(n_start <= n_end,'Start date is after End date');

if (n_start == n_end)
    TraceMessage('Single file',quiet);
end
...
TraceMessage(['Loading ',currency,' rates [',start_date,'-',end_date,'] in ',freq],quiet);
...
for n = (n_start):(n_end)
    ...
    if (~strcmp(last_added_date,added_date))
        R = addDate(R,added_date,quiet);
    end
    last_added_date = added_date;
end

assert(~isempty(R.time),'no data found');

R.freq = freq;
R.start_date = datestr(R.time(1));
R.end_date   = datestr(R.time(end));

TraceMessage('Loaded successfully.',quiet);

if (fix_data)
    R = fixData(R);
    TraceMessage('Data Fixed',quiet);
end
...
%% --- Add-Date-To-Range ---
function new_R = addDate(R, new_date,quiet)
L = length(new_date);
...
try
    ...
    TraceMessage(['Missing ',new_date],quiet,'warning');
    new_R = R;
end
